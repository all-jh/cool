          ...
          function setupRealtimeListeners() {
              const combinedQuery = {
                  posts: db.collection('posts')
                      ...
                      .orderBy('createdAt', 'desc')
                      .limit(20),
                  replies: db.collection('postsrea')
              ...
              // 使用一個監聽器處理所有新增的內容
              const unsubscribePosts = combinedQuery.posts.onSnapshot(snapshot => {
              ...
              const unsubscribeReplies = combinedQuery.replies.onSnapshot(snapshot => {
                  snapshot.docChanges().forEach(change => {
                      if (change.type === 'added') {
...
function setupRealtimeUpdates() {
    // 監聽已批准的貼文變化
    const unsubscribeApproved = db.collection('posts')
        .where('approved', '==', true)
        .onSnapshot(snapshot => {
            snapshot.docChanges().forEach(change => {
                const postData = change.doc.data();
                ...
                if (change.type === 'added' || change.type === 'modified') {
                    ...
                    if (postElement) {
                        ...
                        const approvedContainer = document.getElementById('approvedPostsContainer');
                        approvedContainer.insertBefore(newPostElement, approvedContainer.firstChild);
                    }
                } else if (change.type === 'removed' && postElement) {
                    postElement.remove();
                }
            });
        });
            ...
            snapshot.docChanges().forEach(change => {
                ...
                if (change.type === 'added' || change.type === 'modified') {
                    ...
                    if (replyElement) {
                        ...
                        const approvedContainer = document.getElementById('approvedPostsContainer');
                        approvedContainer.insertBefore(newReplyElement, approvedContainer.firstChild);
                    }
                } else if (change.type === 'removed' && replyElement) {
                    replyElement.remove();
                }
            });
        });
...
// ... existing code ...
          ...
          document.addEventListener('DOMContentLoaded', () => {
              loadPosts();
              const unsubscribeRealtime = setupRealtimeUpdates();
              const unsubscribeAI = setupRealtimeListeners();
          ...
          db.collection('posts')
              ...
              .onSnapshot(snapshot => {
                  snapshot.docChanges().forEach(change => {
                      if (change.type === 'added') {
                          const postData = change.doc.data();
                          console.log('新的主要貼文：', change.doc.id);
                          autoAICheck(change.doc.id, postData.content);
                      }
                  });
              });
          ...
          db.collection('postsrea')
              ...
              .onSnapshot(snapshot => {
                  snapshot.docChanges().forEach(change => {
                      if (change.type === 'added') {
                          const replyData = change.doc.data();
                          console.log('新的回覆貼文：', change.doc.id);
                          autoAICheck(change.doc.id, replyData.replyContent, true);
                      }
                  });
              });
              function createPostElement(docId, postData, isReply) {
                ...
                let content, postNumber, originalPostInfo = '';
                if (isReply) {
                    content = postData.replyContent;
                    postNumber = postData.postNumber ? `#${postData.postNumber}` : '未編號';
                        ...
                        originalPostInfo = `
                            <div class="original-post">
                                <div class="original-post-header">
                                    <span>↳ 回覆貼文 #${postData.originalPost.postNumber}</span>
                                </div>
